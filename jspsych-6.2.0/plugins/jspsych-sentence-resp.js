/**
 * jspsych-sentece-resp
 * a jspsych plugin for free response survey questions
 *
 * Josh de Leeuw (modified: Stephen Van Hedger)
 *
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(trial.questions[i].rows == 1){
        html += '<input type="text" id="sentenceresp" style="font-size:25px;" autofocus name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns+'" value="'+trial.questions[i].value+'"></input>';
      } else {
        html += '<textarea style="font-size:25px;" id="sentenceresp" autofocus name="#jspsych-survey-text-response-' + i + '" cols="' + trial.questions[i].columns + '" rows="' + trial.questions[i].rows + '">'+trial.questions[i].value+'</textarea>';
      }
      html += '</div>';
    }
